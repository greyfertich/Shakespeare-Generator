...
class SentenceGeneration(nn.Module):
  ...
  def forward(self, history, state=None):
    ...
    # Implement here following the given signature                      #
    if state is None:
        state = torch.zeros((history.shape[0], self.hidden_size), device=history.device)
...
def shakespeare_writer():
  ...
  state = None
  pred_char, state = model.forward(inputs, state)
  next_char = sample_next_char_id(pred_char)
  generated_chars.append(index2char[next_char.item()])
  inputs = torch.cat((inputs, next_char.view(1, 1)), dim=1)

  for generation in range(FLAGS.generation_length):
      pred_char, state = model.forward(inputs, state)
      next_char = sample_next_char_id(pred_char)
      inputs = torch.cat((inputs, next_char.view(1, 1)), dim=1)
      generated_chars.append(index2char[next_char.item()])
  with open(start_string[:6] + '.txt', 'w') as file:
      file.write(start_string + ''.join(generated_chars))
